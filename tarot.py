        ...
        if call.data == 'question_callback':
            ...
            for i in statuss:
                if i == bot.get_chat_member(chat_id = my_channel_id, user_id = call.message.chat.id).status:
                    ...
                    button = types.InlineKeyboardButton(text = "Задать мне вопрос", callback_data = 'question_callback')
                    keyboard.add(button)
                    bot.edit_message_text(chat_id = call.message.chat.id, message_id = call.message.message_id, text = f"Привет, `{call.message.chat.first_name}`! Я бот, который раскладывает карты 24/7 и способен ответить на твой любой вопрос в формате `да/нет`. Задай мне подходящий вопрос и я определю твою судьбу в один миг!\n\n*Пример:* `Меня завтра позовут на собеседование?`", parse_mode = 'Markdown', reply_markup = keyboard)
                    break
            else:
